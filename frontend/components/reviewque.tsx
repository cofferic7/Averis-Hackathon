        ...
        <div className="review-app">
            <aside className="sidebar">
                <div className="brand">
                    ...
                    <div><strong>ShipOps</strong><span>Document Control</span></div>
                </div>

                <nav aria-label="Main navigation">
                    <button className={currentPage === "dashboard" ? "active" : ""} onClick={() => { setCurrentPage("dashboard"); setSelectedCase(null); }}><Icon name="home" /><span>Dashboard</span></button>
                    <button className={currentPage === "queue" ? "active" : ""} onClick={() => { setCurrentPage("queue"); setSelectedCase(null); }}><Icon name="queue" /><span>Review Queue</span><b>12</b></button>
                    <button className={currentPage === "resolved" ? "active" : ""} onClick={() => { setCurrentPage("resolved"); setSelectedCase(null); }}><Icon name="check" /><span>Resolved Cases</span></button>
                </nav>

                <div className="sidebar-help">
                    <div className="help-icon">?</div>
                    <strong>Need help?</strong>
                    <p>View the review guide and field rules.</p>
                    <button>Open guide</button>
                </div>
                <p className="version">ShipOps v1.0 · Hackathon demo</p>
            </aside>
            ...
            <style>{`
        *{box-sizing:border-box} body{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#10213f;background:#f4f8fd}.review-app{min-height:100vh;display:grid;grid-template-columns:230px 1fr}.sidebar{position:sticky;top:0;height:100vh;padding:25px 18px 20px;color:#dbeafe;background:linear-gradient(180deg,#071c3d 0%,#0b3564 58%,#0c4a79 100%);display:flex;flex-direction:column;box-shadow:8px 0 30px rgba(14,44,84,.12)}.brand{display:flex;align-items:center;gap:12px;padding:0 8px 34px}.brand-mark{width:38px;height:38px;display:grid;place-items:center;border-radius:11px;color:#fff;font-weight:800;background:linear-gradient(145deg,#2d8cff,#075ddb);box-shadow:0 8px 20px rgba(0,105,255,.35)}.brand strong,.brand span{display:block}.brand strong{color:#fff;font-size:17px}.brand span{margin-top:3px;color:#93afd0;font-size:11px}.sidebar nav{display:grid;gap:8px}.sidebar nav a{height:48px;padding:0 13px;display:flex;align-items:center;gap:13px;border-radius:10px;color:#bcd0e8;text-decoration:none;font-size:14px;transition:.2s}.sidebar nav a:hover{background:rgba(255,255,255,.07);color:#fff;transform:translateX(2px)}.sidebar nav a.active{color:#fff;background:linear-gradient(90deg,#0964ce,#217ce0);box-shadow:0 9px 22px rgba(0,73,174,.35)}.sidebar nav svg{width:19px;height:19px}.sidebar nav b{margin-left:auto;min-width:23px;height:23px;padding:0 6px;display:grid;place-items:center;border-radius:999px;background:rgba(255,255,255,.18);font-size:11px}.sidebar-help{margin-top:auto;padding:16px;border:1px solid rgba(255,255,255,.14);border-radius:14px;background:rgba(255,255,255,.07)}.help-icon{width:27px;height:27px;margin-bottom:10px;display:grid;place-items:center;border-radius:8px;background:#fff;color:#1267c9;font-weight:800}.sidebar-help strong{font-size:13px;color:#fff}.sidebar-help p{margin:6px 0 13px;color:#a9c2de;font-size:11px;line-height:1.5}.sidebar-help button{width:100%;padding:8px;border:0;border-radius:7px;background:rgba(255,255,255,.13);color:#fff;font-weight:700;cursor:pointer}.version{margin:14px 3px 0;color:#87a9cb;font-size:10px}main{min-width:0}.topbar{height:66px;padding:0 31px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #dce6f2;background:rgba(255,255,255,.9);backdrop-filter:blur(12px)}.topbar>div:first-child{display:flex;align-items:center;gap:8px;font-size:12px}.topbar>div:first-child span{color:#8190aa}.topbar>div:first-child svg{width:13px}.user-area{display:flex;align-items:center;gap:10px}.icon-button{position:relative;width:36px;height:36px;border:1px solid #e1e8f1;border-radius:10px;background:#fff;color:#58708f;cursor:pointer}.icon-button svg{width:17px}.icon-button i{position:absolute;right:8px;top:7px;width:6px;height:6px;border-radius:50%;background:#ed4052;border:1px solid white}.avatar{width:36px;height:36px;display:grid;place-items:center;border-radius:50%;background:linear-gradient(145deg,#dcecff,#bbd9fb);color:#13559e;font-size:11px;font-weight:800}.user-area strong,.user-area div span{display:block}.user-area strong{font-size:12px}.user-area div span{font-size:10px;color:#8694a9;margin-top:2px}.down{margin-left:4px;color:#718198}.content{padding:30px;max-width:1500px;margin:auto}.page-heading{display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:22px}.eyebrow{color:#1670d5;font-size:10px;font-weight:800;letter-spacing:1.3px}.page-heading h1{margin:5px 0 4px;font-size:31px;line-height:1.15;letter-spacing:-.8px}.page-heading p{margin:0;color:#687b99;font-size:14px}.refresh{height:39px;padding:0 15px;border:1px solid #cddaea;border-radius:9px;background:#fff;color:#24577f;font-weight:700;cursor:pointer}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px}.stat-card{min-height:112px;padding:20px;display:flex;align-items:center;gap:15px;border:1px solid #dce5f1;border-radius:13px;background:#fff;box-shadow:0 5px 18px rgba(29,63,105,.045);transition:.2s}.stat-card:hover{transform:translateY(-3px);box-shadow:0 12px 27px rgba(29,63,105,.1)}.stat-icon{width:46px;height:46px;display:grid;place-items:center;flex:none;border-radius:14px;font-size:22px;font-weight:800}.stat-card p{margin:0 0 2px;color:#637897;font-size:11px}.stat-card strong{display:inline-block;margin-right:10px;font-size:27px;letter-spacing:-.5px}.stat-card span{display:block;margin-top:3px;color:#91a0b4;font-size:10px}.stat-card.blue .stat-icon{background:#e5f1ff;color:#0b69e8}.stat-card.blue strong{color:#0866df}.stat-card.red .stat-icon{background:#ffe6e9;color:#d9213c}.stat-card.red strong{color:#cc1731}.stat-card.amber .stat-icon{background:#fff0d0;color:#c86c00}.stat-card.amber strong{color:#bd6200}.stat-card.green .stat-icon{background:#daf5e9;color:#087d55}.stat-card.green strong{color:#087852}.queue-panel{overflow:hidden;border:1px solid #d9e4f0;border-radius:14px;background:#fff;box-shadow:0 8px 28px rgba(28,65,108,.055)}.panel-heading{padding:20px 20px 15px;display:flex;align-items:center;justify-content:space-between}.panel-heading h2{margin:0;font-size:17px}.panel-heading p{margin:4px 0 0;color:#8997aa;font-size:11px}.live{padding:6px 9px;border-radius:999px;background:#effaf5;color:#187454;font-size:10px;font-weight:700}.live i{display:inline-block;width:7px;height:7px;margin-right:5px;border-radius:50%;background:#20b47a;box-shadow:0 0 0 4px rgba(32,180,122,.13)}.toolbar{padding:0 20px 17px;display:flex;align-items:center;gap:10px}.search-box{width:min(390px,42%);height:40px;padding:0 11px;display:flex;align-items:center;border:1px solid #d7e1ed;border-radius:9px;background:#fafdff}.search-box:focus-within{border-color:#4389e4;box-shadow:0 0 0 3px rgba(39,119,222,.1)}.search-box svg{width:17px;color:#8092aa}.search-box input{width:100%;height:100%;padding:0 9px;border:0;outline:0;background:transparent;color:#183150}.search-box button{border:0;background:transparent;color:#7b8ca2;font-size:18px;cursor:pointer}.toolbar select,.filter-button{height:40px;padding:0 34px 0 12px;border:1px solid #d7e1ed;border-radius:9px;background:#fff;color:#526781}.filter-button{padding:0 13px;cursor:pointer}.result-count{margin-left:auto;color:#8190a5;font-size:11px}.table-wrap{overflow-x:auto;border-top:1px solid #edf1f6;border-bottom:1px solid #edf1f6}table{width:100%;border-collapse:collapse;min-width:950px}th{padding:13px 16px;text-align:left;background:#f7faff;color:#637793;font-size:10px;text-transform:uppercase;letter-spacing:.35px}td{padding:14px 16px;border-top:1px solid #e7edf5;color:#304764;font-size:12px}tbody tr{transition:.18s}tbody tr:hover{background:#f8fbff}.case-id{display:flex;align-items:center;gap:10px}.case-id>span{width:33px;height:33px;display:grid;place-items:center;border-radius:9px;background:#eaf3ff;color:#1266c6;font-weight:800}.case-id strong,.case-id small{display:block}.case-id small{margin-top:3px;color:#99a5b5;font-size:9px}.issue{display:block;margin-bottom:6px}.priority{display:inline-block;font-size:9px}.priority.high{color:#d3283c}.priority.medium{color:#a86804}.missing{color:#d42f43!important;font-weight:700}.status{display:inline-flex;padding:7px 10px;border-radius:7px;font-size:9px;font-weight:800;text-transform:uppercase;white-space:nowrap}.status.mismatch{background:#ffe1e5;color:#ce1f38}.status.needs_review{background:#ffefcf;color:#a96100}.review-button{min-width:112px;height:36px;padding:0 12px;display:flex;align-items:center;justify-content:center;gap:10px;border:0;border-radius:8px;background:linear-gradient(135deg,#0875ef,#075acb);color:#fff;font-size:11px;font-weight:800;box-shadow:0 6px 13px rgba(0,101,225,.18);cursor:pointer;transition:.2s}.review-button:hover{transform:translateY(-1px);box-shadow:0 9px 18px rgba(0,101,225,.28)}.review-button span{font-size:15px}.empty{padding:45px;text-align:center;color:#667b98}.empty p{font-size:12px}.panel-footer{padding:14px 20px;display:flex;justify-content:space-between;align-items:center;color:#7789a1;font-size:10px}.panel-footer div{display:flex;gap:5px}.panel-footer button{width:29px;height:29px;border:1px solid #dce5ef;border-radius:7px;background:#fff;color:#5a718d;cursor:pointer}.panel-footer button.current{border-color:#1371dc;background:#1371dc;color:#fff}.panel-footer button:disabled{opacity:.4}.modal-backdrop{position:fixed;inset:0;z-index:20;display:grid;place-items:center;padding:20px;background:rgba(5,22,48,.57);backdrop-filter:blur(5px)}.modal{position:relative;width:min(520px,100%);padding:30px;border-radius:18px;background:#fff;box-shadow:0 25px 70px rgba(0,20,50,.3)}.modal-close{position:absolute;right:18px;top:15px;border:0;background:transparent;color:#65758b;font-size:25px;cursor:pointer}.modal h2{margin:6px 0}.modal>p{margin:0;color:#6c7e96}.compare-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:24px 0}.compare-grid div{padding:17px;border:1px solid #dce5ef;border-radius:11px;background:#f8fbff}.compare-grid span,.compare-grid strong{display:block}.compare-grid span{margin-bottom:7px;color:#71839b;font-size:10px}.modal-actions{display:flex;justify-content:flex-end;gap:9px}.modal-actions button{height:40px;padding:0 15px;border-radius:8px;font-weight:800;cursor:pointer}.secondary{border:1px solid #d4deea;background:#fff;color:#536a85}.primary{border:0;background:#0869db;color:#fff}
        @media(max-width:1050px){.review-app{grid-template-columns:82px 1fr}.sidebar{padding:24px 12px}.brand{padding:0 9px 30px}.brand>div:last-child,.sidebar nav span,.sidebar nav b,.sidebar-help,.version{display:none}.sidebar nav a{justify-content:center}.stats{grid-template-columns:repeat(2,1fr)}}
        @media(max-width:700px){.review-app{display:block}.sidebar{position:static;width:100%;height:auto;padding:10px 14px;flex-direction:row;align-items:center}.brand{padding:0;margin-right:auto}.brand>div:last-child{display:none}.sidebar nav{display:flex}.sidebar nav a{height:42px}.sidebar nav span,.sidebar nav b{display:none}.topbar{padding:0 15px}.topbar>div:first-child,.user-area>div:not(.avatar),.down{display:none}.content{padding:20px 14px}.page-heading{align-items:flex-start;gap:15px}.page-heading h1{font-size:25px}.refresh{font-size:0}.refresh:first-letter{font-size:18px}.stats{grid-template-columns:1fr 1fr}.stat-card{padding:14px}.toolbar{flex-wrap:wrap}.search-box{width:100%}.result-count{margin-left:0}.panel-footer{gap:12px}.compare-grid{grid-template-columns:1fr}}
        @media(max-width:430px){.stats{grid-template-columns:1fr}.page-heading p{font-size:12px}.modal-actions{flex-direction:column-reverse}.modal-actions button{width:100%}}
      `}</style>
